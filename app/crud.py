...
def list_transactions(db: Session, limit: int = 100, offset: int = 0):
    stmt = select(models.Transaction).order_by(models.Transaction.date.desc()).limit(limit).offset(offset)
...
def recompute_subscriptions(db: Session):
    ...
    # Get all merchants
    merchants = db.execute(select(models.Transaction.normalized_merchant).distinct()).scalars().all()
    for nm in merchants:
        if not nm:
            continue
        txns = db.execute(
            select(models.Transaction).where(models.Transaction.normalized_merchant == nm).order_by(models.Transaction.date)
        ).scalars().all()
...
def list_subscriptions(db: Session):
    return db.execute(select(models.Subscription).order_by(models.Subscription.next_charge)).scalars().all()

def list_anomalies(db: Session, limit: int = 100):
    return db.execute(select(models.Anomaly).order_by(models.Anomaly.id.desc()).limit(limit)).scalars().all()
